...
// This macro is used to export the OfiStrategy class to the dynamic link library (DLL) that is loaded by the trading engine.
#ifdef _WIN32
...
/**
 * Below are header files that are used by the OfiStrategy class. We just tell the compiler to look for these files.
...
// Class declaration
class OfiStrategy : public Strategy {
public:
    // Constructor & Destructor functions for this class
    OfiStrategy(StrategyID strategyID,
...
private:  
    ...
    */
    void SendOrder(const Instrument* instrument, int quantity, double price);
...
private:
    ...
    int buy_position; // the number of shares of the instrument currently held in the strategy's portfolio.
    int sell_position; // the number of shares of the instrument currently held in the strategy's portfolio.
    int max_buy_position; // the maximum number of shares of the instrument that the strategy can hold.
    int max_sell_position; // the maximum number of shares of the instrument that the strategy can hold.
    int imbalance_threshold; // the threshold for the imbalance
...
extern "C" {
    ...
    _STRATEGY_EXPORTS IStrategy* CreateStrategy(const char* strategyType,
        ...
        if (strcmp(strategyType, GetType()) == 0) {
            return *(new OfiStrategy(strategyID, strategyName, groupName));
        } else {
            return NULL;
        }
    }
