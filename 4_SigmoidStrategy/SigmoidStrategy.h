...
// This macro is used to export the SigmoidStrategy class to the dynamic link library (DLL) that is loaded by the trading engine.
#ifdef _WIN32
...
/**
 * Below are header files that are used by the SigmoidStrategy class. We just tell the compiler to look for these files.
...
// Class declaration
class SigmoidStrategy : public Strategy {
public:
    // Constructor & Destructor functions for this class
    SigmoidStrategy(StrategyID strategyID,
...
private:
    ...
    */
    void SendOrder(const Instrument* instrument, int trade_size_each_time, double ask, double bid);
...
private:
    ...
    double total_stock_position; // the number of shares of the instrument currently held in the strategy's portfolio.
...
extern "C" {
    ...
    _STRATEGY_EXPORTS IStrategy* CreateStrategy(const char* strategyType,
        ...
        if (strcmp(strategyType, GetType()) == 0) {
            return *(new SigmoidStrategy(strategyID, strategyName, groupName));
        } else {
            return NULL;
        }
    }
